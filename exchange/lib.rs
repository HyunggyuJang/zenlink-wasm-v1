...
mod exchange {
    ...
    use ink_env::{
        call,
        call::{build_call, utils::ReturnType, ExecutionInput},
        DefaultEnvironment,
    };
    ...
    impl Exchange {
        ...
        ///
        ///
        /// NOTE: Exchange contract show only be instantiated by Factory.
        ///
        /// #Params
        ///
        ...
        /// - `token_amount`: Amount of token the deployer will transfer from token_account_id to this contract account
        #[ink(constructor)]
        pub fn new(
            ...
            if token_amount > 0 {
                instance.token_transfer_from(deployer, instance.exchange_account_id, token_amount);
            }
        ...
        pub fn add_liquidity(
            ...
            let transferred_value = self.env().transferred_balance();
            assert!(
                deadline >= self.env().block_timestamp() && max_tokens > 0 && transferred_value > 0
            );
            let total_liquidity = self.total_supply;
            let caller = self.env().caller();
            if total_liquidity > 0 {
                assert!(min_liquidity > 0);
                let dot_reserve = self.env().balance() - transferred_value;
                let token_reserve = self.token_balance();
                let token_amount = transferred_value * token_reserve / dot_reserve + 1;
                let liquidity_minted = transferred_value * total_liquidity / dot_reserve;

                assert!(max_tokens >= token_amount && liquidity_minted >= min_liquidity);

                let caller_liquidity = self.balances.get(&caller).unwrap_or(&0u128).clone();
                self.balances
                    .insert(caller, caller_liquidity + liquidity_minted);

                self.total_supply = total_liquidity + liquidity_minted;
                assert!(self.token_transfer_from(caller, self.exchange_account_id, token_amount));

                self.env().emit_event(AddLiquidity {
                    provider: caller,
                    dot_amount: transferred_value,
                    token_amount
                });
        ...
        pub fn remove_liquidity(
            ...
            assert!(
                (amount > 0 && deadline >= self.env().block_timestamp())
                    && (min_dot > 0 && min_token > 0)
            );
            let caller = self.env().caller();
            let total_liquidity = self.total_supply;
            ...
            let token_reserve = self.token_balance();
            let dot_amount = amount * self.env().balance() / total_liquidity;
            let token_amount = amount * token_reserve / total_liquidity;

            assert!((dot_amount > min_dot) && (token_amount > min_token));
            let caller_luquidity = *self.balances.get(&caller).unwrap();
            assert!(caller_luquidity >= amount);

            self.balances.insert(caller, caller_luquidity - amount);
            self.total_supply = total_liquidity - amount;

            self.env()
                .transfer(caller, dot_amount)
                .expect("transfer error");
            self.token_transfer(caller, token_amount);
        ...
        pub fn input_price(
            ...
            let input_amount_with_fee = input_amount * 997u128;
            let numerator = input_amount_with_fee * output_reserve;
            let denominator = (input_reserve * 1000u128) + input_amount_with_fee;
        ...
        pub fn output_price(
            ...
            assert!(input_reserve > 0 && output_reserve > 0);
            let numerator = input_reserve * output_amount * 1000u128;
            let denomiator = (output_reserve - output_amount) * 997u128;
        ...
        fn dot_to_token_input(
            ...
            let token_reserve = self.token_balance();
            let tokens_bought =
                self.input_price(dot_sold, self.env().balance() - dot_sold, token_reserve);
        ...
        /// - `deadline ` : Time after which this transaction can no longer be executed
        #[ink(message, payable)]
        pub fn dot_to_token_swap_input(
            &mut self,
            min_tokens: Balance,
            deadline: Timestamp,
        ) -> Balance {
        ...
        pub fn dot_to_token_transfer_input(
            &mut self,
            min_tokens: Balance,
            deadline: Timestamp,
            recipient: AccountId,
        ...
        fn dot_to_token_output(
            ...
            let dot_sold = self.output_price(
                tokens_bought,
        ...
        /// - `deadline ` : Time after which this transaction can no longer be executed
        #[ink(message, payable)]
        pub fn dot_to_token_swap_output(
            &mut self,
            tokens_bought: Balance,
            deadline: Timestamp,
        ) -> Balance {
        ...
        pub fn dot_to_token_transfer_output(
            &mut self,
            tokens_bought: Balance,
            deadline: Timestamp,
            recipient: AccountId,
        ...
        /// - `deadline`: Time after which this transaction can no longer be executed
        #[ink(message)]
        pub fn token_to_dot_swap_input(
            &mut self,
            tokens_sold: Balance,
            min_dot: Balance,
            deadline: Timestamp,
        ...
        #[ink(message)]
        pub fn token_to_dot_transfer_input(
            &mut self,
            tokens_sold: Balance,
            min_dot: Balance,
            deadline: Timestamp,
        ...
        /// - `deadline`: Time after which this transaction can no longer be executed.
        #[ink(message)]
        pub fn token_to_dot_swap_output(
            &mut self,
            dot_bought: Balance,
            max_tokens: Balance,
            deadline: Timestamp,
        ...
        /// - `recipient`: AcccountId will get the transferred dot.
        #[ink(message)]
        pub fn token_to_dot_transfer_output(
            &mut self,
            dot_bought: Balance,
            max_tokens: Balance,
            deadline: Timestamp,
        ...
        fn token_to_token_input(
            ...
            assert!(
                deadline >= self.env().block_timestamp()
                    && tokens_sold > 0
                    && (min_dot_bought > 0 && min_tokens_bought > 0)
            );
            ...
            let tokens_bought = build_call::<DefaultEnvironment>()
                ...
                .exec_input(
                    ExecutionInput::new(selector_balance_of.into())
                        .push_arg(min_tokens_bought)
                        .push_arg(deadline)
                        .push_arg(recipient),
                )
        ...
        ///
        /// Return the Amount of Token(token_addr) bought
        ///
        /// #Params
        /// - `tokens_sold`: Amount of Tokens sold
        ...
        fn token_to_token_output(
            ...
            assert!(
                exchange_addr != self.exchange_account_id && exchange_addr != AccountId::default()
            );
            ...
            let dot_bought = build_call::<DefaultEnvironment>()
                ...
                .transferred_value(0)
                .exec_input(
                    ExecutionInput::new(selector_dot_to_token_output_price.into())
                        .push_arg(tokens_bought),
                )
                .returns::<ReturnType<Balance>>()
                .fire()
                .unwrap();
            ...
            let selector_dot_to_token_transfer_output =
                call::Selector::new([0x07, 0x83, 0xf4, 0x03]);
            build_call::<DefaultEnvironment>()
                .callee(exchange_addr)
                .gas_limit(self.gas_limit)
                .transferred_value(dot_bought)
                .exec_input(
                    ExecutionInput::new(selector_dot_to_token_transfer_output.into())
                        .push_arg(tokens_bought)
                        .push_arg(deadline)
                        .push_arg(recipient),
                )
        ...
        /// Calculate how many dot it needs to buy a certain amount of token
        ///
        ...
        pub fn token_balance(&mut self) -> Balance {
            ...
            build_call::<DefaultEnvironment>()
                ...
                .exec_input(
                    ExecutionInput::new(selector_balance_of.into())
                        .push_arg(&self.exchange_account_id),
                )
                .returns::<ReturnType<Balance>>()
                .fire()
                .unwrap()
        ...
        pub fn exchange_from_factory(&mut self, token_account: AccountId) -> AccountId {
            ...
            build_call::<DefaultEnvironment>()
                ...
                .transferred_value(0)
                .exec_input(
                    ExecutionInput::new(selector_transfer_from.into()).push_arg(&token_account),
                )
                .returns::<ReturnType<AccountId>>()
                .fire()
                .unwrap()
        }
        ...
        pub fn token_from_factory(&mut self, exchange_addr: AccountId) -> AccountId {
            ...
            build_call::<DefaultEnvironment>()
                ...
                .gas_limit(self.gas_limit / 2)
                .transferred_value(0)
                .exec_input(ExecutionInput::new(selector.into()).push_arg(&exchange_addr))
                .returns::<ReturnType<AccountId>>()
                .fire()
                .unwrap()
        }
...
mod tests {
    ...
    fn test_transfer_liquidity_should_work() {
        ...
        let mut contract =
            Exchange::new(token_account_id, factory_account_id, accounts.alice, 0u128);
    ...
    fn test_output_price() {
        ...
        let sell_amount_2 =
            contract.output_price(output_amount_2, input_reserve_2, output_reserve_2);
        println!(
            "bought amount:{} sell amount:{}",
            output_amount_2, sell_amount_2
        );
    ...
    fn test_input_price() {
        ...
        let output_reserve_2 = 200;
        let bought_amount_2 =
            contract.input_price(input_amount_2, input_reserve_2, output_reserve_2);
        println!(
            "sell amount:{} bought amount:{}",
            input_amount_2, bought_amount_2
        );
